...
void setup_server_search()
{
    ...
    lisener_running_proper = true;
    listener_thread_proper = std::thread(server_searching);
}
...
void end_server_listen()
{
    ...
    lisener_running_proper = false;
    listener_thread_proper.join();
    close(sr_sock);
